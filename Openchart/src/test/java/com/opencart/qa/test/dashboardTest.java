...
@AfterClass
	...
	public void VerifySeachBoxWithValidText() throws IOException {
		String searchdata = ReadData.GetCellValue("dashboard", 1, 0);
		dashboard obj = new dashboard(driver);
		obj.setText(searchdata);
		boolean status=obj.verifyValidProduct();
		Assert.assertTrue(status,"TestScript fail as validproduct Breadcramp is not displayed");
	}
